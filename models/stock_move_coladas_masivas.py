...
class StockPicking(models.Model):
    ...
    def action_cargar_coladas_desde_excel(self):
        ...
        for picking in self:
            ...
            def get_cell(row_values, field_name, fallback_idx=None):
            ...
            for row in rows:
                ...
                move_id_value = get_cell(row, "move_id", 1)
                move_xmlid = get_cell(row, "move_external_id", 2)
                coladas_value = get_cell(row, "sid_coladas_masivo", 11 if len(header_map) > 10 else 9)
